...
public:
    ...
    double integrate(T &func, const double a, const double b, const double tol, void* param);

    double adaptlobstp(T &func, const double a, const double b, const double fa,
                       const double fb, const double is, void* param);
};
...
template <class T>
double AdaptiveIntegrator<T>::integrate(T &f, const double a, const double b, const double tol_, void* param)
{
    double tol, eps;
    ...
    double y[13] = {f(a,param),f(m-x1*h,param),f(m-alpha*h,param),f(m-x2*h,param),f(m-beta*h,param),f(m-x3*h,param),f(m,param),
                    f(m+x3*h,param),f(m+beta*h,param),f(m+x2*h,param),f(m+alpha*h,param),f(m+x1*h,param),f(b,param)
                   };
    ...
    return adaptlobstp(f, a, b, fa, fb, is, param);

}

template <class T>
double AdaptiveIntegrator<T>::adaptlobstp(T &f, const double a, const double b,
        const double fa, const double fb, const double is, void* param)
{
    double m, h;
    m = (a+b)/2.;
    h = (b-a)/2.;
    ...
    {
        return adaptlobstp(f,a,mll,fa,fmll,is,param)
               + adaptlobstp(f,mll,ml,fmll,fml,is,param)
               + adaptlobstp(f,ml,m,fml,fm,is,param)
               + adaptlobstp(f,m,mr,fm,fmr,is,param)
               + adaptlobstp(f,mr,mrr,fmr,fmrr,is,param)
               + adaptlobstp(f,mrr,b,fmrr,fb,is,param);
    }
}
